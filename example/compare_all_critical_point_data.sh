...
# Script to compare critical points between original .dat files and decompressed .dat.SZp.out files
...
if [ $# -lt 4 ]; then
    ...
    echo ""
    echo "Arguments:"
    echo "  data_directory  : Directory containing .dat and .dat.SZp.out files"
...
if [ ! -f "./compare_critical_points" ]; then
    ...
    echo "  g++ -I../SZp/include -I../SZp -fopenmp -o compare_critical_points compare_critical_points.cc ../SZp/.libs/libszp.a -lm -lgomp"
    exit 1
fi

# Find all .dat files (excluding .dat.SZp.out files)
...
for i in "${!dat_files[@]}"; do
    ...
    decompressed_file="${dat_file}.SZp.out"
