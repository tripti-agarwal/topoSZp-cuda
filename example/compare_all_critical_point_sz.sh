...
# Supports .dat.SZ3.out, .dat.zfp.out, .dat.tthresh.out, .dat.fpzip.out, and .dat.sz.out decompressed files
...
if [ $# -lt 4 ]; then
    ...
    echo "                       Supported: .SZ3.out, .zfp.out, .tthresh.out, .fpzip.out, .sz.out"
    echo ""
    echo "Examples:"
    ...
    echo "  $0 ./sz_1E-3 384 320 1e-3 .sz.out"
    exit 1
fi
...
if [ -z "$DECOMPRESSED_SUFFIX" ]; then
    ...
    first_dat=$(find "$DATA_DIR" -maxdepth 1 -type f -name "*.dat" ! -name "*.SZ3.out" ! -name "*.zfp*" ! -name "*.tthresh*" ! -name "*.fpzip*" ! -name "*.sz*" | head -1)
    if [ -n "$first_dat" ]; then
        ...
        elif [ -f "${base_name}.dat.sz.out" ]; then
            DECOMPRESSED_SUFFIX=".sz.out"
            echo "Auto-detected decompressed suffix: .sz.out"
...
elif [[ "$DECOMPRESSED_SUFFIX" == ".sz.out" ]]; then
    mapfile -t dat_files < <(find "$DATA_DIR" -maxdepth 1 -type f -name "*.dat" ! -name "*.sz*" | sort)
else
...
if [ $processed_count -eq 0 ] && [ $failed_count -gt 0 ]; then
    ...
    echo "      $0 $DATA_DIR $ROWS $COLS $ERROR_BOUND .sz.out"
